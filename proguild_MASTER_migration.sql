...
-- ============================================================
-- BLOCK 1b: FIX clients TABLE
-- Root cause: v77-prod-migration.sql created clients with wrong
-- column names (contact_name, contact_phone, contact_email,
-- contact_city, contact_state) and was missing address_line1,
-- zip, tags, preferred_contact entirely.
-- v74-sql.sql has the canonical schema. This block brings any
-- staging/prod instance up to the canonical v74 shape regardless
-- of which migration ran first.
-- Safe to re-run — all use IF NOT EXISTS / DO NOTHING guards.
-- ============================================================

-- 1. Add missing columns (no-ops if already present)
ALTER TABLE clients
...
-- 2. Backfill canonical columns from v77 stale columns where they exist
--    (safe: only updates rows where canonical column is still NULL)
UPDATE clients SET full_name = contact_name
...
UPDATE clients SET phone = contact_phone
  WHERE phone IS NULL
...
UPDATE clients SET email = contact_email
  WHERE email IS NULL
...
UPDATE clients SET city = contact_city
  WHERE city IS NULL
    AND contact_city IS NOT NULL
    AND EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'clients' AND column_name = 'contact_city'
    );

UPDATE clients SET state = contact_state
  WHERE state IS NULL
    AND contact_state IS NOT NULL
    AND EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_name = 'clients' AND column_name = 'contact_state'
    );

-- 3. Default empty tags array where NULL
UPDATE clients SET tags = '{}' WHERE tags IS NULL;

-- 4. Default preferred_contact where NULL
UPDATE clients SET preferred_contact = 'call' WHERE preferred_contact IS NULL;

-- 5. Drop stale v77 columns (only if canonical columns are now populated)
--    Using DO $$ blocks so missing columns are silently ignored.
DO $$ BEGIN
...
DO $$ BEGIN
  ALTER TABLE clients DROP COLUMN IF EXISTS contact_city;
EXCEPTION WHEN undefined_column THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE clients DROP COLUMN IF EXISTS contact_state;
EXCEPTION WHEN undefined_column THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE clients DROP COLUMN IF EXISTS lead_count;
EXCEPTION WHEN undefined_column THEN NULL; END $$;

DO $$ BEGIN
  ALTER TABLE clients DROP COLUMN IF EXISTS total_spent;
EXCEPTION WHEN undefined_column THEN NULL; END $$;

-- 6. Indexes
CREATE INDEX IF NOT EXISTS idx_clients_pro      ON clients (pro_id);
CREATE INDEX IF NOT EXISTS idx_clients_pro_name ON clients (pro_id, full_name);
CREATE INDEX IF NOT EXISTS idx_clients_phone    ON clients (pro_id, phone) WHERE phone IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_clients_email    ON clients (pro_id, email) WHERE email IS NOT NULL;

-- 7. Record fix in migration tracking
INSERT INTO schema_migrations (version, description)
VALUES ('clients_v74_fix', 'Fix clients table: add missing address_line1, zip, tags, preferred_contact; backfill and drop stale v77 columns')
ON CONFLICT (version) DO NOTHING;
