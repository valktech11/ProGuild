...
function buildNav(nl: number, tradeSlug?: string | null, _tradeName?: string | null): NavGroup[] {
  ...
  // Map config emoji keys → clean SVG icons (matches Roofr's design language)
  const NAV_SVG: Record<string, (active: boolean) => React.ReactNode> = {
    '⚡': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>,
    '📋': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="9" x2="15" y2="9"/><line x1="9" y1="13" x2="15" y2="13"/><line x1="9" y1="17" x2="13" y2="17"/></svg>,
    '📅': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>,
    '💬': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>,
    '📝': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>,
    '💰': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>,
    '📈': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>,
    '👤': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
    '🏠': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
    '🛡️': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>,
    '📐': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M3 3l18 18M3 21L21 3"/><path d="M3 3v18h18"/></svg>,
    '💲': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>,
    '⛈️': (a) => <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={a?'#0F766E':'currentColor'} strokeWidth="2" strokeLinecap="round"><path d="M19 16.9A5 5 0 0018 7h-1.26a8 8 0 10-11.62 9"/><polyline points="13 11 9 17 15 17 11 23"/></svg>,
  }
  function navIcon(emoji: string): (a: boolean) => React.ReactNode {
    return NAV_SVG[emoji] ?? (() => <span className="text-base leading-none flex-shrink-0">{emoji}</span>)
  }
            ...
            {session && (
              <div className="flex-shrink-0 px-4 py-4" style={{ borderTop: '1px solid rgba(255,255,255,.07)' }}>
                ...
                {(!session.plan || session.plan === 'Free') && (() => {
                  ...
                  const showCountdown = trialActive && daysLeft <= 30
                  return (
                    <>
                      {showCountdown && (
                        <div style={{ marginBottom: 8, padding: '8px 10px', borderRadius: 10, background: daysLeft <= 7 ? 'rgba(220,38,38,0.15)' : 'rgba(245,158,11,0.15)', border: `1px solid ${daysLeft <= 7 ? 'rgba(220,38,38,0.3)' : 'rgba(245,158,11,0.3)'}` }}>
                          <div style={{ fontSize: 11, fontWeight: 700, color: daysLeft <= 7 ? '#FCA5A5' : '#FCD34D', marginBottom: 2 }}>
                            ⏱ {daysLeft} day{daysLeft !== 1 ? 's' : ''} left in trial
                          </div>
                          <div style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.5)' }}>Upgrade to keep your access</div>
                        </div>
                      )}
                      <a href="/subscribe" className="w-full flex items-center justify-center gap-1.5 py-2 rounded-xl text-[12px] font-bold mb-3 transition-all hover:brightness-110 active:scale-[.98]"
                        style={{ background: 'linear-gradient(135deg,#D97706 0%,#B45309 100%)', color: '#fff', boxShadow: '0 3px 10px rgba(217,119,6,.35)', textDecoration: 'none' }}>
                        {trialActive ? 'Upgrade to Pro' : 'Restore access'}
