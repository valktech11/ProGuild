...
interface ProCardProps {
  pro: Pro & { trade_score?: number; osha_card_type?: string; insurance_status?: string; license_number?: string; is_claimed?: boolean }
  index?: number
}
...
export default function ProCard({ pro, index = 0 }: ProCardProps) {
  ...
  const isClaimed = pro.is_claimed !== false  // default true — only false when explicitly unclaimed
  const licenseNum = (pro as any).license_number || null

  // Unclaimed card — links to claim flow
  if (!isClaimed) {
    return (
      <Link
        href={`/claim/${pro.id}`}
      ...
      >
        ...
        {/* Header — avatar + name (nudged down for banner) */}
        ...
        {/* License number — the key trust signal */}
        {licenseNum && (
  ...
  // Claimed card — existing design unchanged
  return (
    <Link
      href={`/pro/${pro.id}`}
