...
export default function ProCard({ pro, index = 0 }: ProCardProps) {
  ...
  return (
    <Link
      href={`/pro/${pro.id}`}
      className="group block bg-white border border-gray-100 rounded-2xl p-6 hover:border-teal-300 hover:shadow-lg hover:shadow-teal-50 hover:-translate-y-0.5 transition-all duration-200"
    ...
    >
      ...
      <div className="flex gap-4 items-start mb-4">
        ...
        <div
          className="w-12 h-12 rounded-full flex items-center justify-center text-base font-serif flex-shrink-0"
      ...
      {/* Badges */}
      {(pro.is_verified || isPaid(pro.plan_tier)) && (
        <div className="flex gap-2 mb-3">
          ...
          {isElite(pro.plan_tier) && (
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-purple-50 text-purple-800">
              Elite
            </span>
          )}
          {isPaid(pro.plan_tier) && !isElite(pro.plan_tier) && (
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-green-50 text-green-800">
              Pro
            </span>
          )}
        </div>
      ...
      {rating > 0 ? (
        <div className="flex items-center gap-2 mb-4">
          <span className="text-amber-500 text-sm tracking-wide">{starsHtml(rating)}</span>
