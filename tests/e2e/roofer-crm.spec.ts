 ...
 *
 * State is persisted via a JSON file between tests since Playwright
 * runs each test in isolation (module variables reset).
 */
...
type State = {
  ...
  estimateId?:        string
  estimatePublicUrl?: string
  invoiceId?:         string
  invoicePublicUrl?:  string
  gbbEstimateId?:     string
  gbbPublicUrl?:      string
  gbbLeadId?:         string
}

function saveState(update: Partial<State>) {
  const current: State = existsSync(STATE_FILE)
...
// ── Cleanup state file before suite ──────────────────────────────────────────
...
test('TC-04: Create estimate from lead', async ({ page }) => {
  ...
  if (!leadDetailUrl) { test.skip(true, 'TC-03 failed — leadDetailUrl not set'); return }
...
test('TC-05: Send estimate to homeowner', async ({ page }) => {
  ...
  if (!estimateId) { test.skip(true, 'TC-04 failed — estimateId not set'); return }
...
test('TC-06: Public estimate page loads', async ({ page }) => {
  ...
  const estimatePublicUrl = `${BASE_URL}/estimate/${estimateId}`
  saveState({ estimatePublicUrl })
  ...
  await expect(page.getByText(/sign to approve|sign here/i).first()).toBeVisible({ timeout: 10000 })
...
test('TC-09: Invoice auto-created', async ({ page }) => {
  ...
  await expect(
    page.getByText(CLIENT_NAME).or(page.getByText(CLIENT_EMAIL)).or(page.getByText(/INV-\d+/).first())
...
test('TC-12: Record deposit payment', async ({ page }) => {
  ...
  await page.waitForTimeout(3000)
  await expect(
    page.getByText(/recorded|payment.*recorded/i).first()
  ).toBeVisible({ timeout: 10000 })
})
...
test('TC-16: Create GBB estimate', async ({ page }) => {
  ...
  // Save
  await page.getByRole('button', { name: /● save changes|save changes/i }).click()
  await page.waitForTimeout(2000)
...
test('TC-17: GBB public page shows tiers', async ({ page }) => {
  ...
  const gbbPublicUrl = `${BASE_URL}/estimate/${gbbEstimateId}`
  saveState({ gbbPublicUrl })
...
test('TC-18: Homeowner selects tier and signs GBB', async ({ page }) => {
  ...
  await expect(page.getByText(/approved|signed|thank you/i).first()).toBeVisible({ timeout: 15000 })
})
...
test('TC-20: Stripe card payment', async ({ page }) => {
  ...
  if (!stripeKey?.startsWith('sk_test_')) {
    test.skip(true, 'STRIPE_SECRET_KEY not in test mode')
    return
  }
  ...
  if (await page.getByText(/paid in full/i).isVisible()) {
    test.skip(true, 'Invoice already paid')
    return
  }

  const milestone = page.locator('label').filter({ hasText: /deposit|material|completion/i }).first()
  await expect(milestone).toBeVisible({ timeout: 10000 })
...
test.afterAll(async () => {
  ...
  console.log('\n=== E2E Run Summary ===')
  Object.entries(state).forEach(([k, v]) => console.log(`${k}: ${v ?? 'not set'}`))
  console.log('=======================\n')
})
