...
returns void language sql as $$
  update community_posts set like_count = coalesce(like_count, 0) + 1 where id = post_id_param;
$$;

-- RPC to safely decrement like_count (floor at 0)
create or replace function decrement_like_count(post_id_param uuid)
returns void language sql as $$
  update community_posts set like_count = greatest(coalesce(like_count, 0) - 1, 0) where id = post_id_param;
$$;
