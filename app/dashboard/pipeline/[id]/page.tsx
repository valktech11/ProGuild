                      ...
                      <div style={{padding:'18px 20px'}}>
                        {!isEditing&&(
                          <>
                            ...
                            {isRoofing&&(
                              <InsuranceClaimFields key={(lead as any).roofing_job_data?.claim_number ?? lead.id} leadId={lead.id} proId={session!.id} initial={(lead as any).roofing_job_data??{}} darkMode={dk}
                        ...
                        {acts.length===0
                          ...
                          :<div style={{position:'relative'}}>
                            ...
                            {acts.map((item,i)=>{
                              ...
                              const ic=warn?'#EF4444':item.type==='stage'?'#7C3AED':item.type==='note'?'#854F0B':item.type==='quote'?'#0F766E':item.type==='scheduled'?'#64748B':item.type==='invoice_sent'?'#0F766E':item.type==='payment_received'?'#16A34A':['estimate','estimate_sent','estimate_viewed','estimate_approved'].includes(item.type)?'#0F766E':BRAND.teal
                                ...
                                <div key={i} style={{display:'flex',alignItems:'flex-start',gap:14,paddingBottom:i<acts.length-1?18:0,position:'relative',zIndex:1}}>
                                  ...
                                  <div style={{width:30,height:30,borderRadius:'50%',background:ib,border:`2px solid ${ic}25`,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                                    <Svg size={13} stroke={ic}>
                                      ...
                                      {item.type==='payment_received' &&<><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></>}{item.type==='invoice_sent'    &&<><rect x="2" y="3" width="20" height="18" rx="2"/><line x1="2" y1="9" x2="22" y2="9"/><line x1="7" y1="15" x2="12" y2="15"/></>}{item.type==='estimate_sent'    &&<><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></>}
