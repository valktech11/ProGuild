  ...
  async function runSatelliteMeasure() {
    ...
    try{
      ...
      if(meas){
        ...
        // square_count stored for display only — non-authoritative for estimates (Bible §25). User enters SQ in calculator.
        apiFetch(`/api/leads/${lead.id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({pro_id:session.id,pitch:meas.dominantPitch??null,waste_pct:Number(meas.wasteFactor)||null,square_count:Number(meas.totalSquaresOrder)||null})})
                        ...
                        {!isEditing&&(
                          <>
                            ...
                            <div style={{marginTop:(useSpine&&(!isRoofing||isClaim))?0:16,paddingTop:(useSpine&&(!isRoofing||isClaim))?0:16,borderTop:(useSpine&&(!isRoofing||isClaim))?'none':`1px solid ${bdr}`}}>
                              ...
                              {!canViewContact && (
                                <div style={{marginTop:10,marginBottom:8,background:'linear-gradient(135deg,rgba(15,118,110,0.06),rgba(15,118,110,0.03))',border:`1px solid rgba(15,118,110,0.2)`,borderRadius:10,padding:'12px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
                                  <div>
                                    <div style={{fontWeight:700,fontSize:12.5,color:tp,marginBottom:2}}>🔒 Contact details locked</div>
                                    <div style={{fontSize:11.5,color:ts}}>Upgrade to Pro to view phone, email and address.</div>
                                  </div>
                                  <a href="/subscribe" style={{flexShrink:0,padding:'7px 14px',background:'linear-gradient(135deg,#0F766E,#0C5F57)',color:'#fff',borderRadius:7,fontSize:12,fontWeight:700,textDecoration:'none'}}>Upgrade →</a>
                                </div>
                              )}
