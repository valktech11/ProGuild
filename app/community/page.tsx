      ...
      <div className="flex items-center gap-1 px-3 py-2 border-t border-gray-100">
        ...
        {!isOwn && (
          <div className="flex items-center gap-1.5 ml-auto">
            {/* View Profile — icon+text on wide, icon-only on narrow */}
            <Link href={`/community/profile/${post.pro_id}`}
              title="View Profile"
              className="flex items-center gap-1 text-[11px] font-semibold px-2 py-1.5 border border-gray-200 rounded-md text-gray-600 hover:bg-gray-50 transition-colors">
              ...
              <span className="hidden [@container(min-width:320px)]:inline">View Profile</span>
            </Link>
            {/* Request Quote — icon+text on wide, icon-only on narrow */}
            <Link href={`/post-job?pro=${post.pro_id}`}
              title="Request Quote"
              className="flex items-center gap-1 text-[11px] font-semibold px-2 py-1.5 bg-teal-600 text-white rounded-md hover:bg-teal-700 transition-colors">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
              <span className="hidden [@container(min-width:320px)]:inline">Request Quote</span>
            </Link>
          </div>
        )}
      </div>
          ...
          {loading ? (
            ...
            <div className="columns-1 sm:columns-2 gap-4">
              {postsWithLikes.map(post => (
                <div key={post.id} className="break-inside-avoid mb-4" style={{ containerType: 'inline-size' }}>
