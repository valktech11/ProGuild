        ...
        {submitted ? (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            {chosen && renders.find(r => r.viz_skus?.id === chosen) && (
              <div style={{ maxWidth: 400, margin: '0 auto', borderRadius: T.radLg, overflow: 'hidden', border: `1px solid ${t.cardBorder}` }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={renders.find(r => r.viz_skus?.id === chosen)!.render_url}
                  alt="Your chosen roof"
                  style={{ width: '100%', display: 'block' }} />
                <div style={{ padding: '12px 16px', background: t.cardBg, display: 'flex', alignItems: 'center', gap: 10 }}>
                  <div style={{
                    width: 20, height: 20, borderRadius: 4,
                    background: renders.find(r => r.viz_skus?.id === chosen)!.viz_skus.hex_preview,
                    border: '1px solid rgba(0,0,0,0.15)',
                  }} />
                  <span style={{ fontWeight: 700, color: t.textPri, fontSize: 15 }}>
                    {renders.find(r => r.viz_skus?.id === chosen)!.viz_skus.name}
                  </span>
                  <span style={{ marginLeft: 'auto', color: BRAND.teal, fontWeight: 700, fontSize: 13 }}>✓ Your pick</span>
                </div>
              </div>
            )}
          </div>
          ...
          <>
            <div style={{ marginBottom: 28, textAlign: 'center' }}>
              <h1 style={{ fontSize: 26, fontWeight: 800, color: t.textPri, margin: '0 0 10px' }}>
                Pick Your Favourite Shingle
              </h1>
              <p style={{ color: t.textMuted, fontSize: 15, margin: 0 }}>
                Tap the colour you like best. Your roofer will be notified instantly.
              </p>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(renders.length, 3)}, 1fr)`, gap: 16 }}>
              {renders.map(r => (
                <div
                  key={r.viz_skus?.id}
                  onClick={() => handlePick(r.viz_skus?.id)}
                  style={{
                ...
                >
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      background: r.viz_skus?.hex_preview,
                      border: '1px solid rgba(0,0,0,0.15)',
                      flexShrink: 0,
                    }} />
                    <span style={{ fontWeight: 600, color: t.textPri, fontSize: 13 }}>
                      {r.viz_skus?.name}
                    </span>
                    {chosen === r.viz_skus?.id && (
                      <span style={{ marginLeft: 'auto', color: BRAND.teal, fontSize: 13, fontWeight: 700 }}>✓</span>
                    )}
                  </div>
                </div>
              ))}
              ...
              <div style={{ textAlign: 'center', marginTop: 24 }}>
                <button
                  ...
                  style={{
                    ...
                    borderRadius: T.radMd, padding: '13px 36px',
