...
function greeting(name: string): string {
  // Avoid "Hi LLC," or "Hi Corp," — use "Hi there," for business names
  const businessSuffixes = /\b(LLC|Inc|Corp|Co\.|Company|Services|Group|Solutions|Contractors?|Builders?|Construction|Enterprises?)\b/i
  const first = name.split(' ')[0]
  return businessSuffixes.test(name) ? 'Hi there,' : `Hi ${first},`
}
...
function claimEmail(pro: any): string {
  ...
  const licenseNum = pro.license_number ? `<div style="font-size:13px;color:#166534;">• License # ${pro.license_number} — DBPR verified</div>` : ''
...
<table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e2e1db;">
  ...
  <tr><td style="padding:28px 32px;">
    <p style="font-size:14px;color:#555;margin:0 0 16px;">${greeting(pro.full_name)}</p>
    <p style="font-size:14px;color:#555;margin:0 0 20px;">
      We've built a <strong>free verified profile</strong> for you on ProGuild.ai — Florida's professional home for licensed trade contractors.
    ...
    <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:16px;margin-bottom:24px;">
      ...
      <div style="font-size:13px;color:#166534;">• Homeowners and other contractors can contact you directly</div>
      <div style="font-size:13px;color:#166534;">• No subscription, no per-lead fees — always free to claim</div>
    </div>
    <table cellpadding="0" cellspacing="0" style="margin-bottom:20px;">
      <tr>
        <td style="padding-right:12px;">
          <a href="${claimUrl}" style="display:inline-block;background:#0D9488;color:#fff;font-size:14px;font-weight:600;padding:12px 28px;border-radius:10px;text-decoration:none;">Claim my free profile →</a>
        </td>
        <td>
          <a href="${profileUrl}" style="display:inline-block;color:#0D9488;font-size:13px;text-decoration:underline;">View my profile first</a>
        </td>
      </tr>
    </table>
...
export async function GET(req: NextRequest) {
  ...
  const { data: pros, count } = await sb
    .from('pros')
    .select('id, full_name, email, city, state, license_number, trade_category:trade_categories(category_name)', { count: 'exact' })
    .eq('is_claimed', false)
    .eq('email_sent', false)
  ...
  for (let i = 0; i < (pros || []).length; i += 50) {
    ...
    try {
      await getResend().batch.send(batch.map((pro: any) => ({
        ...
        to:      pro.email,
        subject: `${greeting(pro.full_name).replace(',','')} — your free verified profile is live on ProGuild.ai`,
        html:    claimEmail(pro),
      })))
      // Mark as sent
