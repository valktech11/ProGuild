  ...
  if (leadIds.length > 0) {
    ...
    if (propertyIds.length > 0) {
      const { data: reports } = await getSupabaseAdmin()
        ...
        .eq(_callerRole === 'member' ? 'assigned_to_pro_id' : (_companyId ? 'company_id' : 'pro_id'), _callerRole === 'member' ? _callerProId! : (_companyId ?? _callerProId!))
  ...
  try {
    if (!clientId && contact_phone) {
      const { data: byPhone } = await supabase.from('clients').select('id')
        .eq(_callerRole === 'member' ? 'assigned_to_pro_id' : (_companyId ? 'company_id' : 'pro_id'), _callerRole === 'member' ? _callerProId! : (_companyId ?? _callerProId!)).eq('phone', contact_phone.trim()).maybeSingle()
      if (byPhone) clientId = byPhone.id
    }
    if (!clientId && contact_email) {
      const { data: byEmail } = await supabase.from('clients').select('id')
        .eq(_callerRole === 'member' ? 'assigned_to_pro_id' : (_companyId ? 'company_id' : 'pro_id'), _callerRole === 'member' ? _callerProId! : (_companyId ?? _callerProId!)).eq('email', contact_email.toLowerCase().trim()).maybeSingle()
    ...
    if (!clientId && contact_name && streetOnly) {
      const { data: byNameAddr } = await supabase.from('clients').select('id')
        .eq(_callerRole === 'member' ? 'assigned_to_pro_id' : (_companyId ? 'company_id' : 'pro_id'), _callerRole === 'member' ? _callerProId! : (_companyId ?? _callerProId!)).ilike('full_name', contact_name.trim())
  ...
  if (streetOnly) {
    try {
      const { data: existingProp } = await supabase.from('properties').select('id')
        .eq(_callerRole === 'member' ? 'assigned_to_pro_id' : (_companyId ? 'company_id' : 'pro_id'), _callerRole === 'member' ? _callerProId! : (_companyId ?? _callerProId!)).eq('address_line1', streetOnly).maybeSingle()
