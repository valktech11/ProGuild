...
async function sendJobAlertEmails(job: any) {
  ...
  let query = sb.from('pros')
    ...
    .not('email', 'like', '%placeholder.proguild%')
  ...
  const jobUrl = `${process.env.NEXT_PUBLIC_URL || 'https://proguild.ai'}/jobs`
...
<table width="560" cellpadding="0" cellspacing="0" style="max-width:560px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e2e1db;">
  ...
  <tr><td style="padding:28px 32px;">
    ...
    <a href="${jobUrl}" style="display:inline-block;background:#0F766E;color:#fff;font-size:14px;font-weight:600;padding:12px 28px;border-radius:10px;text-decoration:none;">View job &amp; apply</a>
    <p style="font-size:12px;color:#aaa;margin-top:24px;">You're receiving this because you're a claimed pro on ProGuild.ai in ${job.state || 'Florida'}. <a href="${jobUrl}" style="color:#0F766E;">Manage alerts</a></p>
  </td></tr>
</table>
</td></tr>
</table>
