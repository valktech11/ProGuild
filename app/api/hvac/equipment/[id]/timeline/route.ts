...
export async function GET(
  ...
  const [{ data: refrigerantLogs }, { data: measurements }, { data: leads }] = await Promise.all([
    sb.from('hvac_refrigerant_log').select('*').eq('equipment_id', id).eq('pro_id', proId).order('service_date', { ascending: false }),
    sb.from('hvac_equipment_measurements').select('*').eq('equipment_id', id).eq('pro_id', proId).order('measured_at', { ascending: false }).limit(50),
